...
const RecommendationGrid = ({ currentMovieId }) => {
  ...
  if (loading) return <div className="reco-loading">Calcul des affinités cinématographiques...</div>;
      ...
      <div className="reco-grid">
        {recommendations.map((movie) => (
          ...
          <Link to={`/movies/${movie.id}`} key={movie.id} className="reco-card">
            {movie.image ? (
              <img src={getPosterUrl(movie.image)} alt={movie.name} className="reco-thumb" />
            ) : (
              <div className="reco-thumb-placeholder">Pas d'image</div>
            )}
            <h4 className="reco-name">{movie.name}</h4>
            <span className="reco-genre-badge">{movie.genres?.split(',')[0]}</span>
          </Link>
        ))}
      </div>
    </section>
